# PFC-JSONL v3.3 — Technical Documentation

**Product:** PFC-JSONL | **Version:** 3.3 | **By:** ImpossibleForge
...
1. [What is PFC-JSONL?](#1-what-is-pfc-jsonl)
...
PFC-JSONL is a **lossless JSONL compressor** specialized for structured event logs in JSON Lines format — application logs, observability pipelines, audit trails, and any newline-delimited JSON data.

**Key facts:**
- Compresses JSONL event logs to **~9.6% of original size** — less than half the size of gzip
...
**Compression comparison (1 GB JSONL event log):**

| Tool | Ratio | Compress Speed |
|------|-------|----------------|
| **PFC-JSONL default** | **9.6%** | **14.5 MB/s** |
| gzip -6 | 21.7% | ~75 MB/s |
| zstd -3 | 18.8% | ~320 MB/s |
| zstd -19 | 10.0% | 0.45 MB/s |
| bzip2 -9 | 13.1% | ~6.4 MB/s |

> PFC-JSONL archives are **56% smaller than gzip** at competitive speed. zstd-19 matches PFC-JSONL ratio but at 0.45 MB/s — 26 hours per TB.
...
Copy `license.key` into the same directory as your JSONL files.
...
docker run --rm \
  -v $(pwd):/data \
  impossibleforge/pfc-jsonl:v3.3 \
  compress /data/events.jsonl /data/events.pfc
```
...
docker run --rm \
  -v $(pwd):/data \
  impossibleforge/pfc-jsonl:v3.3 \
  decompress /data/events.pfc /data/restored.jsonl
```
...
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  compress /data/events.jsonl /data/events.pfc [OPTIONS]
...
```bash
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  compress /data/events.jsonl /data/events.pfc --level max --verbose
```
...
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  info /data/events.pfc

# Detailed block structure:
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  info /data/events.pfc --blocks
```
...
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  seek-block 1 /data/events.pfc -

# Filter on the fly:
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  seek-block 1 /data/events.pfc - | grep "ERROR"

# To a file:
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  seek-block 1 /data/events.pfc /data/block1.jsonl
```

Useful for spot-checking recent events or sampling a specific time window.

> **Note on block boundaries:** Because JSONL preprocessing tokens can span block edges, seek-block output may differ by a few bytes from the exact corresponding slice of a full decompression. For byte-exact results, use `decompress`.
...
|--------|------------|----------------|-------|----------|
| `fast` | 8 MiB | ~33 MB/s | ~10.5% | Real-time pipelines, quick archiving |
| `default` | 32 MiB | ~14.5 MB/s | ~9.6% | Best balance — recommended |
| `max` | 128 MiB | ~9 MB/s | ~8.2% | Maximum compression, archiving |
...
```bash
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  compress /data/events.jsonl /data/events.pfc --level max
```
...
PFC-JSONL uses block-level random access. Each `.pfc` file is divided into independent compressed blocks. This enables:
...
### Why this is possible with PFC-JSONL but not gzip

PFC-JSONL compresses in **independent blocks** (32 MiB by default). Each block is self-contained — it can be decompressed without any other block. The file header records the compressed size of each block, enabling direct seeking.
...
[Block 1: first chunk of event data]
[Block 2: second chunk of event data]
...
```
...
**Without PFC-JSONL (gzip):** You must download all 10 TB to find those 60 minutes of events.

**With PFC-JSONL v3.3:** The `.pfc.idx` index tells you which blocks contain March 14 14:00-15:00. You download only those blocks (~29 MB). S3 egress: ~$0.003 instead of ~$900. That is a **99.9% reduction**.

---

## 6. Timestamp Index

During compression, PFC-JSONL automatically creates a `.pfc.idx` file alongside the `.pfc` file:

```
events.pfc      ← compressed data
events.pfc.idx  ← timestamp index (auto-generated)
```
...
- Auto-detected timestamp fields: `timestamp`, `ts`, `time`, `@timestamp`

**To compress without generating an index:**

```bash
docker run --rm -v $(pwd):/data impossibleforge/pfc-jsonl:v3.3 \
  compress /data/events.jsonl /data/events.pfc --no-index
```
...
PFC-JSONL v3.3 supports **direct access** to `.pfc` files stored in any cloud storage — without downloading the full archive first, and **without any SDK or cloud credentials inside the container**.
...
You generate a pre-signed URL using your existing cloud tools. The URL carries its own authentication as query parameters. PFC-JSONL uses standard HTTP `Range` requests against that URL to fetch only the required blocks — no SDK, no credentials inside Docker.

**Egress savings:**
- 10 TB archive, 1-hour query → PFC downloads ~29 MB
- gzip equivalent → must download entire 10 TB
- At $0.09/GB: gzip = ~$900 per query. PFC = $0.003.

### Step 1 — Upload both files to cloud storage

```bash
# AWS S3:
aws s3 cp events.pfc      s3://my-bucket/logs/
aws s3 cp events.pfc.idx  s3://my-bucket/logs/

# Azure:
az storage blob upload -f events.pfc      -c logs -n events.pfc
az storage blob upload -f events.pfc.idx  -c logs -n events.pfc.idx
```
...
aws s3 presign s3://my-bucket/logs/events.pfc     --expires-in 3600
aws s3 presign s3://my-bucket/logs/events.pfc.idx --expires-in 3600

# Azure CLI (SAS URL):
az storage blob generate-sas --full-uri \
  --account-name ACCOUNT --container-name logs \
  --name events.pfc --permissions r --expiry 2025-12-31T00:00:00Z

# rclone (all providers):
rclone link remote:my-bucket/logs/events.pfc
```
...
docker run --rm \
  -v $(pwd):/data \
  impossibleforge/pfc-jsonl:v3.3 \
  s3-fetch "https://my-bucket.s3.amazonaws.com/logs/events.pfc?X-Amz-..." \
    --idx-url "https://my-bucket.s3.amazonaws.com/logs/events.pfc.idx?X-Amz-..." \
...
docker run --rm \
  impossibleforge/pfc-jsonl:v3.3 \
  s3-info "https://my-bucket.s3.amazonaws.com/logs/events.pfc?X-Amz-..."
...
docker run --rm \
  ...
  impossibleforge/pfc-jsonl:v3.3 \
  s3-fetch "https://my-bucket.s3.amazonaws.com/logs/events.pfc?X-Amz-..." \
    --blocks 2 3 --out /data/blocks23.jsonl
```
...
- 1 GB JSONL event log (real production data, 3-run average)
...
|------|-------|----------|------------|-------|
| **PFC-JSONL default** | **9.6%** | **14.5 MB/s** | **32 MB/s** | Block-parallel |
| gzip -6 | 21.7% | ~75 MB/s | 200+ MB/s | Single-threaded |
| zstd -3 | 18.8% | ~320 MB/s | 900+ MB/s | Same ratio as gzip! |
| zstd -19 | 10.0% | 0.45 MB/s | 900+ MB/s | 26 hours per TB |
| bzip2 -9 | 13.1% | ~6.4 MB/s | ~35 MB/s | Single-threaded |

**PFC-JSONL archives are 56% smaller than gzip-6.** zstd-19 barely matches PFC-JSONL ratio at 30× slower compress speed.
...
PFC-JSONL compression is **block-parallel** — each worker processes an independent block. Worker count is auto-detected:
...
PFC-JSONL uses a pipeline of algorithms designed specifically for structured event log data:

```
Raw JSONL  →  [Preprocessor]  →  [BWT]  →  [MTF]  →  [RLE]  →  [rANS O1]  →  .pfc
```

1. **Preprocessor** — Tokenizes repetitive JSONL-specific structural patterns into compact tokens before the main compression pass — the primary driver of PFC's compression advantage over general-purpose tools. Reduces data by ~30-40%.
...
4. **RLE (Run-Length Encoding)** — Compresses runs of identical bytes (common after MTF on structured log data).
...
A 10 GB JSONL file compressed at 9.6%:
- Output: ~960 MB
- Working space needed: ~11 GB total
...
PFC-JSONL requires a valid `license.key` file. Place it in the same directory as your JSONL files (the directory mounted as `/data`).
...
PFC-JSONL makes **zero network connections** for compress/decompress/query operations:
...
**Q: What JSONL timestamp fields are supported?**
PFC-JSONL auto-detects the following timestamp fields: `timestamp`, `ts`, `time`, `@timestamp`. The value must be an ISO-8601 string (e.g. `"2025-03-01T14:32:00Z"`). Files without a recognized timestamp field will still compress correctly but time-range queries will fall back to full-file decompression.
...
```bash
docker run impossibleforge/pfc-jsonl:v3.3 --help
docker run impossibleforge/pfc-jsonl:v3.3 compress --help
docker run impossibleforge/pfc-jsonl:v3.3 decompress --help
docker run impossibleforge/pfc-jsonl:v3.3 info --help
docker run impossibleforge/pfc-jsonl:v3.3 seek-block --help
docker run impossibleforge/pfc-jsonl:v3.3 query --help
```

---

*PFC-JSONL v3.3 | ImpossibleForge | impossibleforge@gmail.com*
